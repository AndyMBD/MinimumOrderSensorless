 ...
 ********************************************/
    ...
    void rhf_LascuAndreescus2006_Dynamics(REAL t, REAL *x, REAL *fx){
        ...
        emf[0] = US(0) - CTRL.motor->R*IS(0) + 1*OFFSET_VOLTAGE_ALPHA \
            /*P*/+ 1*VM_LASCU2006_GAIN_P * rotor_flux_error[0] \
            /*I*/+ 1*x[2];
        emf[1] = US(1) - CTRL.motor->R*IS(1) + 1*OFFSET_VOLTAGE_BETA  \
            /*P*/+ 1*VM_LASCU2006_GAIN_P * rotor_flux_error[1] \
            /*I*/+ 1*x[3];
        fx[0] = emf[0];
        fx[1] = emf[1];
        fx[2] = VM_LASCU2006_GAIN_I * rotor_flux_error[0];
        fx[3] = VM_LASCU2006_GAIN_I * rotor_flux_error[1];
    }
    ...
    /* 4. B. Proposed Flux Command Error feedback PI Correction in Controller Frame (xRho) Boldea 2008 */
    void rhf_Boldea2008_Dynamics(REAL t, REAL *x, REAL *fx){
        ...
        emf[0] = US(0) - CTRL.motor->R*IS(0) + OFFSET_VOLTAGE_ALPHA \
            /*P*/+ VM_BOLDEA2008_GAIN_P * rotor_flux_error[0] \
            /*I*/+ x[2];
        emf[1] = US(1) - CTRL.motor->R*IS(1) + OFFSET_VOLTAGE_BETA  \
            /*P*/+ VM_BOLDEA2008_GAIN_P * rotor_flux_error[1] \
            /*I*/+ x[3];
        fx[0] = emf[0];
        fx[1] = emf[1];
        fx[2] = VM_BOLDEA2008_GAIN_I * rotor_flux_error[0];
        fx[3] = VM_BOLDEA2008_GAIN_I * rotor_flux_error[1];
    }
    void VM_Boldea2008(){
        /* Proposed VM based Flux Command Error Feedback Correction in Controller Frame (xRho), implemented in AB frame + ODE4 */
        // stator flux and integral states update
        general_4states_rk4_solver(&rhf_Boldea2008_Dynamics, CTRL.timebase, FE.boldea.x, CL_TS);
        // Unpack x
        FE.boldea.psi_1[0]                    = FE.boldea.x[0];
        FE.boldea.psi_1[1]                    = FE.boldea.x[1];
        FE.boldea.correction_integral_term[0] = FE.boldea.x[2];
        FE.boldea.correction_integral_term[1] = FE.boldea.x[3];
        FE.boldea.u_offset[0] = FE.boldea.correction_integral_term[0];
        FE.boldea.u_offset[1] = FE.boldea.correction_integral_term[1];
        // rotor flux updates
        FE.boldea.psi_2[0] = FE.boldea.psi_1[0] - LSIGMA*IS_C(0);
        FE.boldea.psi_2[1] = FE.boldea.psi_1[1] - LSIGMA*IS_C(1);
        FE.boldea.psi_2_ampl = sqrt(FE.boldea.psi_2[0]*FE.boldea.psi_2[0]+FE.boldea.psi_2[1]*FE.boldea.psi_2[1]);
    }
